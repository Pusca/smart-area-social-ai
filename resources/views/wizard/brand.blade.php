...
<div id="alter-ego-section" class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
    ...
    @if($alterEgos->isEmpty())
        <div class="mt-5 flex flex-col items-center rounded-2xl border border-dashed border-gray-300 bg-gray-50 px-6 py-10 text-center">
            ...
            <p class="mt-3 text-sm font-medium text-gray-700">Nessun alter ego creato</p>
            <p class="mt-1 max-w-sm text-xs text-gray-500">Carica foto, video e audio della persona: l'AI genererà contenuti che le assomiglino visivamente e ne rispettino la voce.</p>
            <a href="{{ route('alter-ego.create') }}" class="mt-4 inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700">
                Crea il primo alter ego
            </a>
        </div>
    @else
        <div class="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div class="flex items-center gap-3 rounded-2xl border border-gray-200 bg-gray-50 p-3.5 transition hover:border-indigo-200 hover:bg-indigo-50/30">
                ...
                @if($aeFirstPhoto && \Illuminate\Support\Facades\Storage::disk('public')->exists($aeFirstPhoto))
                    <img src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($aeFirstPhoto) }}" alt="{{ $ae->name }}" class="h-12 w-12 shrink-0 rounded-full object-cover ring-2 ring-white shadow-sm">
                @else
                    <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-indigo-100 to-cyan-100 text-indigo-500">
                        <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zm-4 7a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                    </div>
                @endif
                <div class="min-w-0 flex-1">
                    <div class="flex items-center gap-1.5">
                        <p class="truncate text-sm font-semibold text-gray-900">{{ $ae->name }}</p>
                        @if($ae->is_default)
                            <span class="shrink-0 rounded-full bg-indigo-100 px-1.5 py-0.5 text-[10px] font-semibold text-indigo-700">Default</span>
                        @endif
                    </div>
                    <p class="mt-0.5 text-xs text-gray-500">
                        {{ \App\Services\AlterEgoService::archetypeLabel((string) $ae->archetype) }}
                        @if(count($aePhotos) > 0)
                            · <span class="text-emerald-600">{{ count($aePhotos) }} foto</span>
                        @else
                            · <span class="text-amber-600">Nessuna foto</span>
                        @endif
                    </p>
                </div>
                <a href="{{ route('alter-ego.edit', $ae) }}" class="shrink-0 rounded-xl border border-gray-200 bg-white p-1.5 text-gray-500 hover:text-indigo-600 transition" title="Modifica">
